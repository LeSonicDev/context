...
- **上下文扩充 (Context Expansion Strategy)**：
    - **标准保留**：修改行上下各保留至少 3 行原始代码。
    - **结构锚定 (Structural Anchoring)**：这是 **IDE 兼容性的关键**。如果修改点位于函数、类或逻辑块的**末尾**，上下文必须**强制向下延伸**，直到包含**下一个代码块的定义行**（如 `def next_func():`）或**文件末尾**。
    - **严禁悬空**：绝不允许 Diff 结束于缩进变化的代码行（如 `return` 或 `pass`），必须展示该 Block 结束后的空行及后续内容。
- **缩进保真 (Indentation Fidelity)**：**严格继承**源代码的缩进风格（空格数/Tab）。对于 Python 代码，修改行的缩进必须与上下文的缩进逻辑完美对齐，**严禁**目测估算。

## 3. 反复检查 (Check)
- **行号校验**：自检 Hunk Header 中的起始行号是否与当前认知的代码物理行号完全一致。
- **空白字符敏感度 (Whitespace Sensitivity)**：**这是最关键的检查点**。Diff 中 Context 行（未修改行）的前导空格必须与原始代码在**字节级**完全一致。
- **防截断自检**：确认 Hunk 的末尾没有切断代码结构。如果 Hunk 结束在两个函数之间，必须包含这两个函数之间的空行。
